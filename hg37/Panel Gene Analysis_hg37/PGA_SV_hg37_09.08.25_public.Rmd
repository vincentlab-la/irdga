---
title: "PGA_SV_hg37_07.25 - Panel-Gene SV analysis for hg37 builds"
author: "Layla Ahmed"
date: "2025-06-24"
...
setup <- function(){
  messages <- c(
  "You are currently using the Structural Variant (SV) filter code specifically for Panel Gene Analysis in hg37.",
  "If you want to specifically look through the whole genome in your file, please go to WGA_SV_hg37.",
...
CNVFilter <- function(){
  ...
  if (("otgErdsPctFreq_50pctRecOvlp" %in% names(currentdata))) {
    ...
    cnvRow <<- nrow(currentdata)
    cnvText <<- "CNVator & ERDS Database Filter <= 1%"
    currentdata_cnv <<-currentdata
...
merged_data <<- merge(currentdata_genic, ref_name_filt_unique, by.x = "gene_symbol", by.y ="gene_symbol")
}
...
sheet_name_1 <<- as.character("Filtered_MANTA")
  ...
  sheet_name_panel_sep <<- as.character("Panel_MANTA (Gene Sep)")
  
  addWorksheet(wb, sheet_name_1)
  addWorksheet(wb, sheet_name_2)
  addWorksheet(wb, sheet_name_panel)
  addWorksheet(wb, sheet_name_panel_sep)
  addWorksheet(wb, sheet_name_3)
  addWorksheet(wb, sheet_name_4)
  
  
  
  currentdata_panel_sep <<- currentdata_panel %>%
    group_by(CHROM, SV_Start, SV_Stop) %>%
  summarise(
    gene_symbol = paste(unique(gene_symbol), collapse = ","), # Combine gene_symbol
    across(everything(), ~ dplyr::first(.)), # Keep the first value for other columns
  ...
  currentdata_MANTA_Save <<- currentdata_MANTA_Save %>%
    group_by(CHROM, SV_Start, SV_Stop) %>%
  summarise(
    gene_symbol = paste(unique(gene_symbol), collapse = ","), # Combine gene_symbol
    across(everything(), ~ dplyr::first(.)), # Keep the first value for other columns
  ...
  Genes_row <<-  nrow(currentdata_genic_MANTA)
  ...
  Genesv_row <<-  nrow(currentdata_geneSV_MANTA)
  Genesv_text <<- "Unique SVs (Genic)"
  
  FilterStep_MANTA$Step <<- c(FilterStep_MANTA$Step,Genesv_text,Genes_text)
  FilterStep_MANTA$Count <<- c(FilterStep_MANTA$Count,Genesv_row, Genes_row) 
  ...
  writeData(wb, sheet = sheet_name_1, x = currentdata_MANTA_Save, na.string = "NA")
  writeData(wb, sheet = sheet_name_2, x = currentdata_genic_MANTA, na.string = "NA")
  writeData(wb, sheet = sheet_name_panel_sep, x = currentdata_panel_sep)
...
sheet_name_panel_sep <<- as.character("Panel_CNV (Gene Sep)")
...
addWorksheet(wb, sheet_name_4)
  ...
  currentdata_panel_CNV_sep <<- currentdata_panel_CNV %>%
    group_by(CHROM, SV_Start, SV_Stop) %>%
  summarise(
    gene_symbol = paste(unique(gene_symbol), collapse = ","), # Combine gene_symbol
    across(everything(), ~ dplyr::first(.)), # Keep the first value for other columns
  ...
  currentdata_CNV_Save <<- currentdata_CNV_Save %>%
    group_by(CHROM, SV_Start, SV_Stop) %>%
  summarise(
    gene_symbol = paste(unique(gene_symbol), collapse = ","), # Combine gene_symbol
    across(everything(), ~ dplyr::first(.)), # Keep the first value for other columns
  ...
  writeData(wb, sheet = sheet_name_1, x = currentdata_CNV_Save)
  ...
  writeData(wb, sheet = sheet_name_panel_sep, x = currentdata_panel_CNV_sep)

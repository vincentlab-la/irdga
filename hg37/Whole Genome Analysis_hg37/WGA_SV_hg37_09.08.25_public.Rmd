---
title: "WGA_SV_hg37_07.25 - Whole Genome SV analysis for hg37 builds"
author: "Layla Ahmed"
date: "2025-06-24"
...
#You are currently using the Structural Variant (SV) filter code specifically for Whole Genome Analysis in hg37.
#If you want to specifically look for panel genes in your file, please go to PGA_SV_hg37.
...
setup <- function(){
  messages <- c(
  "You are currently using the Structural Variant (SV) filter code specifically for Whole Genome Analysis in hg37.",
  "If you want to specifically look for panel genes in your file, please go to PGA_SNV_hg37.",
  "This script was built to filter MANTA & CNVnator files in parallel;",
  "If you opt to load in your SV files through File -> Import Dataset -> Base (readr), please name your files 'patientdata_MANTA' & 'patientdata_CNV' so the script can recognise them."
)
...
MANTAFilter <- function(){
  ...
  MANTAotgRow <<- nrow(currentdata)
  MANTAotgText <<- "MANTA Database (1000G) Filter <= 1%"
  currentdata_MANTA_otg <<-currentdata
  } else if (("otgMantaPercFreq_90percRecipOverlap" %in% names(currentdata))) {
    currentdata <<- dplyr::filter(currentdata, otgMantaPercFreq_90percRecipOverlap %in% NA | otgMantaPercFreq_90percRecipOverlap <=1) 
         MANTAotgRow <<- nrow(currentdata)
         MANTAotgText <<- "MANTA Database (1000G) Filter <= 1%"
         currentdata_MANTA_otg <<-currentdata
  } else{
     MANTAotgRow <<- as.character("Not Applicable")
     MANTAotgText <<- "MANTA Database (1000G) Filter <= 1%"
     currentdata_MANTA_otg <<-currentdata
  ...
  FilterStep_MANTA$Step <<- c(FilterStep_MANTA$Step, CleanText,MANTAotgText, MANTAText, DellyText, DRGNText, DGVText)
  FilterStep_MANTA$Count <<- c(FilterStep_MANTA$Count,CleanRow,MANTAotgRow, MANTARow, DellyRow, DRGNRow, DGVRow)
...
CNVFilter <- function(){
  ...
  if (("otgErdsPctFreq_50pctRecOvlp" %in% names(currentdata))) {
    ...
    cnvText <<- "CNVator & ERDS Database (1000G) Filter <= 1%"
    currentdata_cnv <<-currentdata
  } else if (("erdsPercFreq_50percRecipOverlap" %in% names(currentdata))){
    currentdata <<- dplyr::filter(currentdata, erdsPercFreq_50percRecipOverlap %in% NA | erdsPercFreq_50percRecipOverlap <=1) 
    currentdata <<- dplyr::filter(currentdata, cnvnatorPercFreq_50percRecipOverlap %in% NA | cnvnatorPercFreq_50percRecipOverlap <=1) 
    cnvRow <<- nrow(currentdata)
    cnvText <<- "CNVator & ERDS Database (1000G) Filter <= 1%"
    currentdata_cnv <<-currentdata
  }
  ...
  if (("gnomAD_SF_max50pctRecOvlp" %in% names(currentdata))){
   ...
   if (("CGparentalPercFreq_50percRecipOverlap" %in% names(currentdata))){
    currentdata <<-  dplyr::filter(currentdata, CGparentalPercFreq_50percRecipOverlap %in% NA | CGparentalPercFreq_50percRecipOverlap <=1)
    CGRow <<- nrow(currentdata)
    CGText <<- "CG Parental Filter <= 1%"
    currentdata_CG <<-currentdata
  }  else {
    CGRow <<- as.character("Not Applicable")
    CGText <<- "CG Parental Filter <= 1%"
    currentdata_CG <<-currentdata
  }
...
FilterStep_CNVnator$Step <<- c(FilterStep_CNVnator$Step, CleanText, cnvText, gnomADText, CGText, DRGNText, DGVText)
FilterStep_CNVnator$Count <<- c(FilterStep_CNVnator$Count, CleanRow, cnvRow, gnomADRow, CGRow, DRGNRow, DGVRow)
...
prepfiles <- function(){
RefSeqURL <- "https://hgdownload.soe.ucsc.edu/goldenPath/hg19/database/ncbiRefSeq.txt.gz"
directory <- getwd()
RefSeqName <- paste0(directory, "/ncbiRefSeq.txt.gz")
download.file(RefSeqURL,RefSeqName)
RefSeq_19 <- read.table(gzfile(RefSeqName))
colnames(RefSeq_19)[c(1:13)] <- c("bin", "name", "chrom", "strand", "txstart", "txEnd", "cdsStart", "cdsEnd", "ExonCount", "exonStarts", "exonEnds", "score", "gene_symbol", "cdsStartStat", "cdsEndStat", "ExonFrames")
RefSeq_19 <- RefSeq_19 %>%
  dplyr::select(-score)
# Calculate how far each gene match is from the actual variant

ref_name_filt <- RefSeq_19[!grepl("alt", RefSeq_19$chrom), ]
ref_name_filt <- ref_name_filt[!grepl("fix", ref_name_filt$chrom), ]
ref_name_filt_unique <- ref_name_filt %>%
  distinct(gene_symbol, chrom, cdsStart, cdsEnd, ExonCount, .keep_all = TRUE)
ref_name_filt_unique_filtered <<- ref_name_filt_unique %>%
  group_by(gene_symbol) %>%  
  slice_max(ExonCount, with_ties = FALSE) %>%  # Keep only the highest ExonCount per gene_symbol
  ungroup()

merged_data <<- merge(currentdata_genic, ref_name_filt_unique_filtered, by.x = "gene_symbol", by.y ="gene_symbol")
}
...
CDSMA_row <<-  nrow(variants_with_exon_overlaps_easy_MANTA)
  ...
  FilterStep_MANTA$Step <<- c(FilterStep_MANTA$Step,Genesm_text,Genesvm_text,CDSMA_text)
  FilterStep_MANTA$Count <<- c(FilterStep_MANTA$Count,Genesm_row,Genesvm_row, CDSMA_row) 

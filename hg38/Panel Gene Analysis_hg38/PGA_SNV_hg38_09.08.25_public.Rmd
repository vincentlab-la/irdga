...
```{r}
#You are currently using the Single Nucleotide Variant (SNV) filter code specifically for  Panel Gene Analysis in hg38.
#If you want to specifically look through the whole genome in your file, please go to WGA_SNV_hg38.
...
setup <- function(){
  messages <- c(
  "You are currently using the Single Nucleotide Variant (SNV) filter code specifically for  Panel Gene Analysis in hg38.",
  "If you want to pecifically look through the whole genome in your file, please go to WGA_SNV_hg38.",
  "If you are starting from the original TCAG file, please go to File -> Import Dataset -> Base (readr) as the file upload codeblock in this script will struggle to upload such a huge file.",
  "Please name your file 'patientdata' so the script can recognise it."
)
...
scoringpanelSNVs <- function() {
  ...
  predVote = suppressWarnings(cbind(
    ...
    as.numeric(panel_filtered_data$REVEL_score) >= user_cutoffs$REVEL_cutoff))
